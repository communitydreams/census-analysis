...
with formcol2:
    location_type = st.radio("Select Location Type", ["ZIP Code", "Census Tract"], key='location_type')
            ...
            with st.spinner(f"Fetching data for {location_type} {location_value}..."):
                try:
                    if location_type == "ZIP Code":
                        ...
                        data = process.fetch_and_prepare_data(location_value, census_tract=True)
...
if data:
    ...
    with col2:    
        st.markdown(f"<h2 style='text-align: center'>{data['location_string']}</h2> <br><br>", unsafe_allow_html=True)
